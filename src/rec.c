...
void rec(const int Vmax, struct objects_t * object_set, struct retained_t * bagpack) {
	...
	for(int obj_idx = first; obj_idx < nb_objects; obj_idx++) { // on va d'objet en objet en commençant à l'index donné pour éviter de prendre les objets déjà pris
		struct object_t * ptr_object = &(object_set->objects[obj_idx]); // on récupère l'objet courrant
		int curr_volume = Vmax - ptr_object->volume; // on ajoute le volume de l'objet courrant à celui du sac que l'on a déjà
		if(curr_volume >= 0) { // on vérifie si le nouveau volume est bien onferieur à celui maximal du sac
			...
			if(duplicata->utilities_sum > best_bagpack->utilities_sum) { // on vérifie si l'utilité du nouveau sac est supérieur à l'ancien sac 
				...
				bagcpy(best_bagpack, duplicata); // on copie le nouveau meilleur sac dans le meilleur sac
			}
			#ifdef _TRACE_
